...
?>
	...
	<html>
		<head>
			...
			<title>
				Servidor Brawl Network WorldMap
			</title>
			<script language="javascript" type="text/javascript">
				function show_tip(header,guild,jobinfo,levels,map,jobid)
				{
					...
					wtip.innerHTML = '<table width="150" border="0" cellspacing="0" cellpadding="0"\><tr class=\'tip_header\'\><td\>' + header + '</td\></tr\><tr class=\'tip_guild\'\><td\>' + guild + '</td\></tr\><tr class=\'tip_text\'\><td\><img src="./images/jobs/' + jobid + '.gif"></td\></tr\><tr class=\'tip_text\'\><td\>' + jobinfo + '</td\></tr\><tr class=\'tip_text\'\><td\>' + levels + '</td\></tr\><tr class=\'tip_text\'\><td\>' + map + '</td\></tr\><\/table\>';
...
$query = sprintf(WORLDMAP);
$result = execute_query($query, "AjaxMapper.php");
	...
	while ($char = $result->fetch_row())
	{
		...
		{ // Mapa configurado en coordenadas
			echo '
				<img src="http://cp.brawlnetwork.net/images/' . (($char[9] == 'M')?'roM':'roF') . '.gif" style="position:absolute; border:0px; left:' . rand($maps[$map]['x1'], $maps[$map]['x2']) .'px; top: ' . rand($maps[$map]['y1'], $maps[$map]['y2']) .'px;" onMouseMove="show_tip(\'' . htmlformat($char[1]) . '\',\'' . htmlformat($char[8]) . '\',\'' . $jobs[$char[7]] . '\',\'(' . $char[5] . '/' . $char[6] . ')\',\'' . htmlformat($char[4]) . '\',\'' . $char[7] . '\');" onMouseOut="hide_tip();">
			';
		}
	}

	echo '
			<img src="http://cp.brawlnetwork.net/worldmap.jpg">
		</div>
	';
?>
