...
#include "buying.h" // struct s_vending
#include "mob.h"
...
struct map_session_data {
	...
	struct {
		...
		unsigned rest : 1;
		unsigned storage_flag : 3; //0: closed, 1: Normal Storage open, 2: guild storage open [Skotlex] 3: Rent Storage [ZephStorage]
		...
		unsigned showcast :1; // [Zephyrus] Display casting time to user
		unsigned showcastdelay :1; // [Zephyrus] Display casting delay time to user
		unsigned showexp :1;
		unsigned showgain :1;
		unsigned showzeny :1;
		...
		unsigned battleinfo : 1;
		unsigned short displaydrop;
		unsigned pvpmode : 1; // PK Mode [Zephyrus]
		unsigned noks : 3; // [Zeph Kill Steal Protection]
		unsigned secure_items : 1; // [Zephyrus] Item Security
		...
		unsigned only_walk : 1; // [Zephyrus] Block Skills and Item usage to a player
		...
		unsigned cashshop : 1; // [CashShop System]
		unsigned view_mob_info : 1;
		short view_aura, user_aura;
		unsigned evade_antiwpefilter : 1; // Required sometimes to show the user previous to use the skill
		unsigned harmony_block : 1; // Harmony attack block security
		bool mac_changed;
	} state;
	struct {
	...
	int gmlevel;
	char mac_address[24]; // To control Double Logins
	...
	struct s_itemdelay itemdelay[MAX_ITEMUSAGE_DELAY];
	...
	// Harmony v3
	struct s_harmony_map_data *harmony;
...
#define pc_cant_act(sd)       ( (sd)->npc_id || (sd)->vender_id || (sd)->buyer_id || (sd)->chatID || ((sd)->sc.opt1 && (sd)->sc.opt1 != OPT1_BURNING) || (sd)->state.trading || (sd)->state.storage_flag )
...
bool pc_authok(struct map_session_data* sd, int, time_t, int gmlevel, const char* mac_address, struct mmo_charstatus* status, bool mac_changed);
