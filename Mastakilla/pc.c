 ...
 *------------------------------------------*/
bool pc_authok(struct map_session_data *sd, int login_id2, time_t expiration_time, int gmlevel, const char* mac_address, struct mmo_charstatus *st, bool mac_changed)
{
	int i;
	...
	sd->gmlevel = gmlevel;
	strncpy(sd->mac_address, mac_address, 24);
	sd->state.mac_changed = mac_changed;
	...
	for( i = 0; i < MAX_ITEMUSAGE_DELAY; i++ )
		sd->itemdelay[i].usage_tick = tick;
...
int pc_reg_received(struct map_session_data *sd)
{
	...
	sd->state.user_aura = pc_readglobalreg(sd,"USERAURA");
	// Cash shop
	...
	if( sd->state.mac_changed )
	{
		sd->state.mac_changed = false;
		clif_disp_onlyself(sd, msg_txt(905), strlen(msg_txt(905)));
		clif_disp_onlyself(sd, msg_txt(906), strlen(msg_txt(906)));
		clif_disp_onlyself(sd, msg_txt(907), strlen(msg_txt(907)));
		clif_disp_onlyself(sd, msg_txt(908), strlen(msg_txt(908)));
	}
 ...
 * Point Shop
 *------------------------------------------*/
int pc_countitem(struct map_session_data *sd, short nameid)
{
	int i;
	...
	if( i < MAX_INVENTORY )
		return sd->status.inventory[i].amount;

	return 0;
}

int pc_payitem(struct map_session_data *sd, short nameid, int amount)
{
	int i;
...
int pc_dropitem(struct map_session_data *sd,int n,int amount)
{
	...
	if(sd->status.inventory[n].nameid <= 0 ||
		...
		sd->status.inventory[n].amount < amount ||
		sd->state.trading || sd->vender_id != 0 || sd->buyer_id != 0 ||
	...
	if ((item_data= itemdb_exists(sd->status.inventory[n].nameid)) != NULL)
		if (item_data->type == IT_WEAPON || item_data->type == IT_CARD || item_data->type == IT_ARMOR) {
			char output[100];
			sprintf(output, "[- Has tirado %s al suelo -]", item_data->jname);
			clif_displaymessage (sd->fd, output);
		}
...
int pc_useitem(struct map_session_data *sd,int n)
{
	...
	if( sd->state.only_walk || sd->state.harmony_block )
		return 0;
	...
	if( sd->inventory_data[n]->flag.delay_consume && ( sd->ud.skilltimer != -1 /*|| !status_check_skilluse(&sd->bl, &sd->bl, ALL_RESURRECTION, 0)*/ ) )
		return 0;
	if( sd->inventory_data[n]->usage_delay )
	{
		int i;
		delay = sd->inventory_data[n]->usage_delay;
		nameid = sd->status.inventory[n].nameid;

		ARR_FIND(0,MAX_ITEMUSAGE_DELAY,i,sd->itemdelay[i].nameid == sd->status.inventory[n].nameid);
		if( i < MAX_ITEMUSAGE_DELAY )
		{
			if( DIFF_TICK(sd->itemdelay[i].usage_tick,tick) > 0 )
				return 0;
			sd->itemdelay[i].nameid = 0;
		}
	}
	...
	{
		int i;
		ARR_FIND(0,MAX_ITEMUSAGE_DELAY,i,!sd->itemdelay[i].nameid || DIFF_TICK(sd->itemdelay[i].usage_tick,tick) <= 0);
		if( i < MAX_ITEMUSAGE_DELAY )
		{
			sd->itemdelay[i].nameid = nameid;
			sd->itemdelay[i].usage_tick = tick + delay;
		}
		else
			ShowWarning("pc_useitem: No more free slots to set Item Usage Delay. Increase MAX_ITEMUSAGE_DELAY value in itemdb.h.\n");
	}
...
int pc_setpos(struct map_session_data* sd, unsigned short mapindex, int x, int y, uint8 clrtype)
{
	...
	else if( !map[m].flag.battleground && map_flag_vs(m) && sd->state.harmony_block )
	{
		mapindex = sd->status.save_point.map;
		x = sd->status.save_point.x;
		y = sd->status.save_point.y;
...
int pc_checkskill(struct map_session_data *sd,int skill_id)
{
	...
	if( skill_get_blocked(skill_id) && !pc_isGM(sd) )
		return 0;
...
int pc_checkbaselevelup(struct map_session_data *sd)
{
	...
	do {
		...
		if (battle_config.use_statpoint_table)
			next = stats_point_table[sd->status.base_level] - stats_point_table[sd->status.base_level-1];
		else //Estimated way.
...
void pc_record_mobkills(struct map_session_data *sd, struct mob_data *md)
{
	...
	if( !sd ) return;
	if( map[sd->bl.m].flag.battleground && sd->state.bg_id )
	{
		int i;
...
void pc_record_maxdamage(struct block_list *src, struct block_list *dst, int damage)
{
	...
	{
		if( map[src->m].flag.battleground && sd->state.bg_id && sd->status.bgstats.top_damage < damage )
...
void pc_record_damage(struct block_list *src, struct block_list *dst, int damage)
{
	...
	switch( dst->type )
	{
		...
		case BL_MOB:
		{
			...
			if( map[src->m].flag.battleground && sd->state.bg_id && md->class_ >= 2100 && md->class_ <= 2104 )
...
void pc_calc_ranking(struct map_session_data *tsd, struct map_session_data *ssd, int skill)
{
	...
	else if( ssd->state.bg_id && map[m].flag.battleground && tsd->state.bg_id )
	{
		/*==========================================
		...
		ssd->state.bg_kills++; // This BG Kills
	}
...
/*	if( pc_isriding(sd, OPTION_MADO) )
	...
	pc_setreg(sd,add_str("@killerrid"),src?src->id:0);
	if( sd->state.bg_id )
	{
		struct battleground_data *bg;
		if( map[sd->bl.m].flag.battleground && (bg = bg_team_search(sd->state.bg_id)) != NULL && bg->die_event[0] )
		{
			pc_setreg(sd,add_str("@killer_bg_id"),bg_team_get_id(src)); // Killer's Team
			if( ssd && ssd->state.bg_id )
				pc_setreg(sd,add_str("@killer_bg_src"),ssd->bl.id);
			else
				pc_setreg(sd,add_str("@killer_bg_src"),0);

			npc_event(sd, bg->die_event, 0);
			if( ssd && ssd->state.bg_id != sd->state.bg_id && !strcmpi(map[sd->bl.m].name,"bat_b03") )
			{ // Team DeathMatch
				add2limit(sd->status.bgstats.td_deaths, 1, USHRT_MAX);
				if( ssd->state.bg_id && (bg = bg_team_search(ssd->state.bg_id)) != NULL )
				{
	...
	if( ssd )
	{
		...
		pc_setreg(ssd,add_str("@killedrid"), sd->bl.id);
...
#if PACKETVER >= 20091110
				clif_displayexp(sd, exp, 1, false);
#endif
...
#if PACKETVER >= 20091110
				clif_displayexp(sd, exp, 2, false);
#endif
...
int pc_unequipitem(struct map_session_data *sd,int n,int flag)
{
	...
	if( (sd->status.inventory[n].equip & EQP_ARMS) && sd->weapontype1 == 0 && sd->weapontype2 == 0 && (!sd->sc.data[SC_SEVENWIND] || sd->sc.data[SC_ASPERSIO]) ) //Check for seven wind (but not level seven!)
...
int pc_get_aura(struct map_session_data *sd)
{
	...
	if( sd->state.pvpmode )
		return 15;
	if( sd->state.user_aura != 0 )
		return sd->state.user_aura; // Priority Custom Aura
