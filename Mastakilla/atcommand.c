...
int atcommand_partybuff(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
    ...
    if( !sd->status.party_id ) {
    	clif_displaymessage(fd, "No estás en party"); // You're not in a party.
    	return -1;
    }
...
int atcommand_load(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if ((map[sd->bl.m].flag.pvp) && (40 >= pc_isGM(sd))) { // El Numero en rojo, quiere decir que solo los GM con el Nivel 10 o superior podrán utilizar el comando en los mapas.
	...
	if ((map[sd->bl.m].flag.gvg) && (40 >= pc_isGM(sd))) { // El Numero en rojo, quiere decir que solo los GM con el Nivel 10 o superior podrán utilizar el comando en los mapas.
...
int atcommand_aura(const int fd, struct map_session_data* sd, const char* command, const char* message)
{
	...
	if ((map[sd->bl.m].flag.battleground) && (40 >= pc_isGM(sd))) { // El Numero en rojo, quiere decir que solo los GM con el Nivel 10 o superior podrán utilizar el comando en los mapas.
