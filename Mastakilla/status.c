...
void initChangeTables(void)
{
	...
	StatusIconChangeTable[SC_SERVICE4U] = SI_SERVICE4U;
...
int status_damage(struct block_list *src, struct block_list *target, int hp, int sp, int walkdelay, int flag, int skill)
{
	struct status_data *status;
			...
			{ // Check for Target change defenses
				...
				if( !(flag&32) && (sce = sc->data[SC__SHADOWFORM]) && skill != HW_GRAVITATION )
				{
					...
					if( d_bl )
					{
						...
						status_damage(NULL,d_bl,hp,0,0,flag|32,0);
					}
					if( --(sce->val3) <= 0 )
...
int status_heal(struct block_list *bl,int hp,int sp, int flag)
{
	...
	if (hp < 0) {
		status_damage(NULL, bl, cap_value(-hp, INT_MIN, INT_MAX), 0, 0, 1, 0);
		hp = 0;
	}
	...
	if(sp < 0) {
		status_damage(NULL, bl, 0, cap_value(-sp, INT_MIN, INT_MAX), 0, 1, 0);
		sp = 0;
	}
...
int status_base_amotion_pc(struct map_session_data* sd, struct status_data* status)
{
	...
	{ // baseaspd + floor(sqrt((agi^2/2) + (dex^2/5))/4 + (potskillbonus*agi/200) + shieldpenalty)
		...
		amotion += sd->aspd_add; // Currently only Masamune do this
		if( sc->data[SC_OTHILA] && sc->data[SC_OTHILA]->val2 )
			amotion -= sc->data[SC_OTHILA]->val2; // Fixed 4 ASPD
	}
	else
	{
		// base weapon delay
...
int status_calc_pc_(struct map_session_data* sd, bool first)
{
	...
	sd->max_weight = max_weight_base[pc_class2idx(sd->status.class_)] + sd->status.str * 300; // STR + 30 Weight
		...
		if( sd->inventory_data[index] )
		{
			...
			for( j = 0; j < MAX_SLOTS; j++ )
			{
				...
				if( j >= sd->inventory_data[index]->slot && !itemdb_isenchant(c) )
					continue;
				if( !c || (data = itemdb_exists(c)) == NULL )
					continue;
...
void status_calc_bl_main(struct block_list *bl, enum scb_flag flag)
{
	...
	{
		status->batk = status_base_atk(bl,status);
		if( (temp = b_status->batk - status_base_atk(bl,b_status)) )
		{
			temp += status->batk;
			status->batk = cap_value(temp, 0, USHRT_MAX);
		}
...
static short status_calc_aspd_rate(struct block_list *bl, struct status_change *sc, int aspd_rate)
{
	...
	if(!sc->data[SC_QUAGMIRE])
	{
		...
		if( (!battle_config.renewal_system_enable && sc->data[SC_BERSERK]) || sc->data[SC_SATURDAYNIGHTFEVER])
			aspd_rate -= 300;
...
//		break;
	...
	if( sd && sd->state.view_aura && !battle_config.anti_mayapurple_hack && (type == SC_HIDING || type == SC_CLOAKING || type == SC_CLOAKINGEXCEED || type == SC__INVISIBILITY || type == SC_CHASEWALK) )
...
#define sc_timer_next(t,f,i,d) \
	...
	case SC_TOXIN:
		...
		{ //Damage is every 10 seconds including 3%sp drain.
			...
			status_damage(NULL,bl,1,status->max_sp*3/100,0,16,0);
			flag = !sc->data[type];
			map_freeblock_unlock();
			if( !flag )
				sc_timer_next(10000 + tick, status_change_timer, bl->id, data );
...
int status_readdb(void)
{
	...
	if( !battle_config.renewal_system_enable )
		...
		sprintf(path, "%s/job_db1_renewal.txt", db_path);
	...
	while(fgets(line, sizeof(line), fp))
	{
		...
		char* split[6 + MAX_WEAPON_TYPE];
		i++;
		if(line[0]=='/' && line[1]=='/')
			continue;
		for(j=0,p=line; j < 6 + MAX_WEAPON_TYPE && p; j++){
			split[j]=p;
			p=strchr(p,',');
			if(p) *p++=0;
		}
		if(j < 5 + battle_config.renewal_system_enable + MAX_WEAPON_TYPE)
		...
		if( battle_config.renewal_system_enable )
			shield_penalty[class_]=atoi(split[5]); // Renewal Shield Penalty
		for( j = 0; j < MAX_WEAPON_TYPE; j++ )
			aspd_base[class_][j]=atoi(split[j+5+battle_config.renewal_system_enable]);
	}
	fclose(fp);
	...
	if( !battle_config.renewal_system_enable )
		...
		sprintf(path, "%s/refine_db_renewal.txt", db_path);
