...
void clif_storagelist(struct map_session_data* sd, struct storage_data* stor)
{
	...
	bufe = (unsigned char*)aMallocA(MAX_STORAGE * se + 4);
	...
	for( i = 0, n = 0, ne = 0; i < MAX_STORAGE; i++ )
	{
		...
		{ //Equippable
			...
			clif_item_sub(bufe, ne*se+6, &stor->items[i], id, id->equip);
			clif_addcards(WBUFP(bufe, ne*se+16), &stor->items[i]);
#if PACKETVER >= 20071002
			WBUFL(bufe,ne*se+24)=stor->items[i].expire_time;
			WBUFW(bufe,ne*se+28)=stor->items[i].bound ? 2 : 0;
#endif
#if PACKETVER >= 20100629
			if (id->equip&EQP_HELM)
...
void clif_ext_storagelist(struct map_session_data* sd, struct extra_storage_data* stor)
{
	struct item_data *id;
	int i,n,ne;
	unsigned char *buf;
	...
	const int se = 28;
#endif
	buf = (unsigned char*)aMallocA(MAX_EXTRA_STORAGE * s + 4);
	bufe = (unsigned char*)aMallocA(MAX_EXTRA_STORAGE * se + 4);

	for( i = 0, n = 0, ne = 0; i < MAX_EXTRA_STORAGE; i++ )
	{
		if( stor->items[i].nameid <= 0 )
			continue;
		id = itemdb_search(stor->items[i].nameid);
...
void clif_guildstoragelist(struct map_session_data *sd,struct guild_storage *stor)
{
	...
	bufe = (unsigned char*)aMallocA(MAX_GUILD_STORAGE * se + 4);
	...
	for( i = 0, n = 0, ne = 0; i < MAX_GUILD_STORAGE; i++ )
	{
		...
		{ //Equippable
			WBUFW(bufe,ne*se+4)=i+1;
			clif_item_sub(bufe, ne*se+6, &stor->storage_[i], id, id->equip);
			clif_addcards(WBUFP(bufe, ne*se+16), &stor->storage_[i]);
#if PACKETVER >= 20071002
			WBUFL(bufe,ne*se+24)=stor->storage_[i].expire_time;
			WBUFW(bufe,ne*se+28)=stor->storage_[i].bound ? 2 : 0;
#endif
#if PACKETVER >= 20100629
			if (id->equip&EQP_HELM)
				WBUFW(bufe,ne*se+30)= id->look;
 ...
 * カプラ倉庫のアイテム数を更新
 *------------------------------------------*/
void clif_updatestorageamount(struct map_session_data* sd, int amount, int max)
{
	int fd;
...
int clif_skill_damage(struct block_list *src,struct block_list *dst,unsigned int tick,int sdelay,int ddelay,int damage,int div,int skill_id,int skill_lv,int type)
{
	...
	if( sd && skill_id == CR_ACIDDEMONSTRATION )
	{
		if( damage > 0 )
		{
			...
			else if( map[src->m].flag.battleground && sd->state.bg_id )
...
void clif_viewequip_ack(struct map_session_data* sd, struct map_session_data* tsd)
{
	...
	for(i=0,n=0; i < MAX_INVENTORY; i++)
	{
		...
		WFIFOW(fd, n*s+67) = tsd->status.inventory[i].bound ? 2 : 0;
#if PACKETVER >= 20100629
...
void clif_parse_WantToConnection(int fd, TBL_PC* sd)
{
	...
	// Harmony always uses packet version 1
	packet_ver = 1;
	...
	if( !harm_process_login_request(fd, sd) )
	{
		aFree(sd);
...
void clif_parse_LoadEndAck(int fd,struct map_session_data *sd)
{
	...
	sd->state.view_aura = pc_get_aura(sd);
	...
	if( map[sd->bl.m].flag.battleground )
	{
		if( sd->state.changemap )
		{
			if( map[sd->bl.m].flag.battleground == 3 )
				clif_set01D6(sd,6); // PVP Mode to enable display of pvp score board
			else
			{
				clif_set01D6(sd,19); // Battleground Mode - enable top score board
	...
	if( ( (battle_config.pet_no_gvg && map_flag_gvg(sd->bl.m)) || map[sd->bl.m].flag.ancient ) && sd->pd )
...
//		skill_unit_move(&sd->pd->bl,gettick(),1);
	...
	if(sd->state.connect_new) {
		...
		clif_updatestatus(sd,SP_DEX);
		clif_updatestatus(sd,SP_LUK);
		// Filtro Baby GW [Warfare]
		if ((map[sd->bl.m].flag.babygw) && !(sd->class_&JOBL_BABY)) {
		pc_setpos(sd, mapindex_name2id(MAP_PRONTERA), 156, 178, 0);
		clif_displaymessage(fd, "[Baby WOE] Solo los Baby Class pueden entrar a las Baby WOEs, ser疽 expulsado.");
		}
...
void clif_parse_ActionRequest_sub(struct map_session_data *sd, int action_type, int target_id, unsigned int tick)
{
	...
	case 0x07: // continuous attack
		...
		if( pc_cant_act(sd) || sd->sc.option&OPTION_HIDE || sd->state.harmony_block )
			return;
...
void clif_cashshop_show(struct map_session_data *sd, struct npc_data *nd)
{
	...
	{
		WFIFOL(fd,4) = pc_countitem(sd, nd->cashitem); // Item Count
	...
	for( i = 0; i < nd->u.shop.count; i++ )
	{
		...
		{ // Discount Price for Premium Account (10% discount)
			val = (unsigned int)((double)nd->u.shop.shop_item[i].value * (double)(80/100.));
			if( val <= 0 ) val = 1;
			WFIFOL(fd,16+i*11) = val;
		}
		else
			WFIFOL(fd,16+i*11) = nd->u.shop.shop_item[i].value;
...
void clif_parse_cashshop_buy(int fd, struct map_session_data *sd)
{
	...
	if( nd->subtype == SPSHOP )
	{
		WFIFOL(fd,2) = pc_countitem(sd, nd->cashitem);
		WFIFOL(fd,6) = 0;
	}
	else
	{
...
void clif_mercenary_info(struct map_session_data *sd)
{
	...
	WFIFOW(fd,6) = cap_value(temp, 0, SHRT_MAX);
	temp = status_calc_matk_max(&md->bl,status);
	WFIFOW(fd,8) = cap_value(temp, 0, SHRT_MAX);
...
int clif_bg_memberlist(struct map_session_data *sd)
{
	...
	for( i = 0, c = 0; i < bg->count; i++ )
	{
		...
		WFIFOL(fd,c*104+22) = psd->state.bg_kills; // Exp slot used to show kills
 ...
 * Buying Store
 *------------------------------------------*/
void clif_buyingstore_open(struct map_session_data *sd, int num)
{
	int fd;
	nullpo_retv(sd);
	fd = sd->fd;
	...
	WFIFOW(fd,0) = 0x810;
	WFIFOW(fd,2) = min(num,5); // Max slots are 5
	WFIFOSET(fd,packet_len(0x810));
}

void clif_parse_buyingstore_open(int fd, struct map_session_data *sd)
{
	short len = (short)RFIFOW(fd,2) - 89, count = len / 8;
	unsigned int zenylimit = RFIFOL(fd,4);
	bool flag = (bool)RFIFOB(fd,8);
	...
	if( battle_config.super_woe_enable )
	{
		clif_displaymessage (sd->fd, "Buying not available on Super WoE / GvG Events.");
		return;
	}
	if( sd->state.secure_items )
	{
	...
	if( message[0] == '\0' || !flag || count < 1 ) // invalid input
		return;

	buying_open(sd,message,zenylimit,data,count);
}

void clif_buyingstore_opened(struct map_session_data *sd, int id, struct s_buying *buying)
{
	int i,fd,count;
	nullpo_retv(sd);
...
int clif_parse(int fd)
{
	...
	if( packet_db[packet_ver][cmd].func != NULL )
	{
		...
		if( sd && sd->bl.prev == NULL && packet_db[packet_ver][cmd].func != clif_parse_LoadEndAck && (cmd < 0x6A0 || cmd >= 0x6BF) ) // Allow harmony packets
...
static int packetdb_readdb(void)
{
	...
	struct {
		...
		{harm_parse_GrfHashblockAck,"harm_grfblockack"},
		{harm_parse_Notification,"harm_notification"},
		...
		{clif_parse_buyingstore_open,"buyingstore_open"},
		{NULL,NULL}
	};
